...
function Inventory({ products, refresh, editable }: any) {
  ...
  const open = async (product: any) => { if (String(product.id).startsWith('sample-')) return; setSelected(product); setAdjustment(''); setReason(''); setModalError(''); const result = await supabase.from('inventory_adjustments').select('*').eq('product_id', product.id).order('created_at', { ascending: false }).limit(10); if (result.error) setModalError(result.error.message); setHistory(result.data || []); };
  const save = async (e: React.FormEvent) => { e.preventDefault(); if (!selected || !editable) return; const quantity = Number(adjustment); if (!adjustment.trim() || !Number.isInteger(quantity) || quantity === 0) { setModalError('Enter a non-zero whole-number adjustment quantity.'); return; } if (!reason.trim()) { setModalError('A reason is required.'); return; } setBusy(true); setModalError(''); const previous = Number(selected.stock || 0); const { data: userData, error: userError } = await supabase.auth.getUser(); if (userError || !userData.user?.email) { setModalError(userError?.message || 'Could not identify the authenticated user.'); setBusy(false); return; } const next = previous + quantity; if (next < 0) { setModalError('Adjustment cannot reduce stock below zero.'); setBusy(false); return; } const inserted = await supabase.from('inventory_adjustments').insert({ product_id: selected.id, previous_quantity: previous, adjustment_quantity: quantity, new_quantity: next, reason: reason.trim(), adjusted_by: userData.user.email }); if (inserted.error) { setModalError(inserted.error.message); setBusy(false); return; } const updated = await supabase.from('products').update({ stock: next, updated_at: new Date().toISOString() }).eq('id', selected.id); if (updated.error) { setModalError(updated.error.message); setBusy(false); return; } setBusy(false); setSelected(null); await refresh(); };
  ...
  return <Page title="Inventory" sub="Monitor current stock levels across the catalogue.">
    ...
    {selected && <div className="modal-backdrop"><form className="modal" onSubmit={save}><button type="button" className="modal-close" onClick={() => setSelected(null)}>×</button><span className="modal-kicker">INVENTORY ADJUSTMENT</span><h3>{selected.name}</h3><p className="modal-help">Current stock: <b>{Number(selected.stock || 0)} {selected.unit || selected.stock_unit || 'PCS'}</b></p><label>Adjustment quantity<input type="number" step="1" required value={adjustment} onChange={e => setAdjustment(e.target.value)} placeholder="e.g. 5 or -2" disabled={!editable || busy} /></label><label>Reason<textarea required value={reason} onChange={e => setReason(e.target.value)} placeholder="Why is this stock changing?" disabled={!editable || busy} /></label><h4>Recent adjustments</h4><div className="modal-help">{history.length ? history.map((item: any) => <div key={item.id || item.created_at}><b>{Number(item.adjustment_quantity) > 0 ? '+' : ''}{item.adjustment_quantity}</b> → {item.new_quantity} · {item.reason} <small>({item.adjusted_by || '—'}, {item.created_at ? new Date(item.created_at).toLocaleString('en-IN') : '—'})</small></div>) : 'No adjustments recorded yet.'}</div>{modalError && <div className="error-banner">{modalError}</div>}{editable && <button className="primary wide" disabled={busy}>{busy ? 'Saving…' : 'Save adjustment'}</button>}</form></div>}
  </Page>;
}
