...
type Tab = 'Dashboard' | 'Orders' | 'Inventory' | 'Customers' | 'Abandoned Carts' | 'Invoices' | 'Delivery slots' | 'Team';
type Area = Tab;
...
const areas: Area[] = ['Dashboard', 'Orders', 'Inventory', 'Customers', 'Abandoned Carts', 'Invoices', 'Delivery slots', 'Team'];
  ...
  return <div className="shell"><aside className={mobile ? 'open' : ''}><div className="logo"><span>J</span><div><b>JAB<span>WE</span>MEAT<sup>™</sup></b><small>ADMIN CONSOLE</small></div></div>
    ...
    {mobile && <div className="scrim" onClick={() => setMobile(false)} />}<main><header><button className="menu" onClick={() => setMobile(true)}><Menu /></button><div><h1>{tab}</h1><p>Live staging · {new Date().toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p></div><label><Search /><input placeholder="Search orders, SKU, customers…" /></label><button className="bell"><Bell /><i /></button><button className="store" onClick={() => window.open('../apps/jabwemeat-store/index.html', '_blank')}>View store <ArrowUpRight /></button></header>
      <section className="content">{error && <div className="error-banner page-error">{error}<button onClick={() => { setError(''); loadAll(); }}><RefreshCw /> Retry</button></div>}{loading && <div className="sync-note">Syncing live data…</div>}
        {tab === 'Dashboard' && <Overview active={activeOrders.length} value={todayValue} products={products.filter(p => p.active !== false).length} carts={carts.length} low={low} orders={orders} onTab={setTab} profileName={profileName} canInventoryEdit={canEdit('Inventory')} canOrdersView={canView('Orders')} />}
        ...
        {tab === 'Customers' && <Customers />}{tab === 'Abandoned Carts' && <AbandonedCarts carts={carts} refresh={loadAll} editable={canEdit('Abandoned Carts')} />}{tab === 'Invoices' && <Upcoming title="Invoices" />}
      </section></main></div>;
}
...
function Inventory({ products, categories, refresh, editable }: any) {
  const sample = [
  ...
  const [selected, setSelected] = useState<any>(null); const [history, setHistory] = useState<any[]>([]); const [adjustment, setAdjustment] = useState(''); const [reason, setReason] = useState(''); const [modalError, setModalError] = useState(''); const [busy, setBusy] = useState(false); const [productOpen, setProductOpen] = useState(false); const [productBusy, setProductBusy] = useState(false); const [productError, setProductError] = useState(''); const [productForm, setProductForm] = useState<any>({ name: '', sku: '', category_id: categories[0]?.id || '', description: '', weight: '', servings: '', price: '', mrp: '', stock: '0', low_stock_threshold: '10', image_url: '', featured: false, active: true });
  ...
  const setProduct = (key: string, value: any) => setProductForm((draft: any) => ({ ...draft, [key]: value }));
  const addProduct = async (e: React.FormEvent) => { e.preventDefault(); if (!editable) return; setProductError(''); const price = Number(productForm.price), mrp = Number(productForm.mrp); if (!productForm.name.trim() || !productForm.sku.trim() || !productForm.category_id) { setProductError('Name, SKU and category are required.'); return; } if (!Number.isFinite(price) || price < 0 || !Number.isFinite(mrp) || mrp < price) { setProductError('Enter a valid price and an MRP equal to or higher than price.'); return; } setProductBusy(true); const result = await supabase.from('products').insert({ name: productForm.name.trim(), sku: productForm.sku.trim().toUpperCase(), slug: productForm.name.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') + '-' + productForm.sku.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-'), category_id: productForm.category_id, description: productForm.description.trim(), weight: productForm.weight.trim(), servings: productForm.servings.trim(), price, mrp, stock: Math.max(0, Math.floor(Number(productForm.stock) || 0)), low_stock_threshold: Math.max(0, Math.floor(Number(productForm.low_stock_threshold) || 0)), image_url: productForm.image_url.trim() || null, featured: !!productForm.featured, active: !!productForm.active }).select().single(); if (result.error) { setProductError(result.error.message); setProductBusy(false); return; } setProductBusy(false); setProductOpen(false); setProductForm({ name: '', sku: '', category_id: categories[0]?.id || '', description: '', weight: '', servings: '', price: '', mrp: '', stock: '0', low_stock_threshold: '10', image_url: '', featured: false, active: true }); await refresh(); };
  const toggleProduct = async (product: any) => { if (!editable || String(product.id).startsWith('sample-')) return; const result = await supabase.from('products').update({ active: !product.active }).eq('id', product.id); if (result.error) { setModalError(result.error.message); return; } await refresh(); };
  return <Page title="Inventory" sub="Monitor current stock levels across the catalogue." action={editable ? <button className="primary" onClick={() => { setProductError(''); setProductForm((draft: any) => ({ ...draft, category_id: draft.category_id || categories[0]?.id || '' })); setProductOpen(true); }}><Plus /> Add Product</button> : undefined}>
    <div className="table-wrap basic-inventory"><table><thead><tr><th>Product name</th><th>SKU</th><th>Current stock</th><th>Unit</th><th>Minimum stock</th><th>Status</th><th>Catalogue</th><th>Last updated</th><th>View</th></tr></thead><tbody>
      {rows.map((p: any) => { const stock = Number(p.stock || 0), minimum = Number(p.minimum || 0), state = status(stock, minimum); return <tr key={p.id || p.sku}><td><b>{p.name}</b></td><td>{p.sku || '—'}</td><td>{stock}</td><td>{String(p.unit).toUpperCase()}</td><td>{minimum}</td><td><span className={`status ${state === 'In Stock' ? 'in-stock' : state === 'Out of Stock' ? 'out-of-stock' : ''}`}>{state}</span></td><td><button className={`catalogue-toggle ${p.active !== false ? 'enabled' : 'disabled'}`} disabled={!editable || String(p.id).startsWith('sample-')} onClick={() => toggleProduct(p)}>{p.active !== false ? 'Enabled' : 'Disabled'}</button></td><td>{p.updated ? new Date(p.updated).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}</td><td><button className="view-order" disabled={String(p.id).startsWith('sample-')} onClick={() => open(p)}><Eye /> View</button></td></tr>; })}
    </tbody></table></div>
    {productOpen && <div className="modal-backdrop"><form className="modal product-modal" onSubmit={addProduct}><button type="button" className="modal-close" onClick={() => setProductOpen(false)}>×</button><span className="modal-kicker">CATALOGUE PRODUCT</span><h3>Add Product</h3><div className="two"><label>Product name<input required value={productForm.name} onChange={e => setProduct('name', e.target.value)} placeholder="e.g. Chicken Leg Curry Cut" /></label><label>SKU<input required value={productForm.sku} onChange={e => setProduct('sku', e.target.value)} placeholder="JWM-CHK-003" /></label></div><div className="two"><label>Category<select required value={productForm.category_id} onChange={e => setProduct('category_id', e.target.value)}><option value="">Select category</option>{categories.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label>Weight / pack<input value={productForm.weight} onChange={e => setProduct('weight', e.target.value)} placeholder="500 g" /></label></div><label>Description<textarea value={productForm.description} onChange={e => setProduct('description', e.target.value)} placeholder="Short customer-facing description" /></label><div className="two"><label>Price (₹)<input required type="number" min="0" step="0.01" value={productForm.price} onChange={e => setProduct('price', e.target.value)} /></label><label>MRP (₹)<input required type="number" min="0" step="0.01" value={productForm.mrp} onChange={e => setProduct('mrp', e.target.value)} /></label></div><div className="two"><label>Opening stock<input type="number" min="0" step="1" value={productForm.stock} onChange={e => setProduct('stock', e.target.value)} /></label><label>Low-stock threshold<input type="number" min="0" step="1" value={productForm.low_stock_threshold} onChange={e => setProduct('low_stock_threshold', e.target.value)} /></label></div><label>Image URL<input type="url" value={productForm.image_url} onChange={e => setProduct('image_url', e.target.value)} placeholder="Optional approved image URL" /></label><div className="product-checks"><label className="check"><input type="checkbox" checked={productForm.featured} onChange={e => setProduct('featured', e.target.checked)} /> Featured on storefront</label><label className="check"><input type="checkbox" checked={productForm.active} onChange={e => setProduct('active', e.target.checked)} /> Enabled for customers</label></div>{productError && <div className="error-banner">{productError}</div>}<button className="primary wide" disabled={productBusy}>{productBusy ? 'Adding…' : 'Add Product'}</button></form></div>}
...
function AbandonedCarts({ carts, refresh, editable }: any) {
  const [selected, setSelected] = useState<any>(null); const [query, setQuery] = useState('');
  const rows = (carts || []).filter((c: any) => ['active','abandoned','recovered','expired'].includes(c.status)).filter((c: any) => `${c.id} ${c.customer_name || ''} ${c.email || ''} ${c.mobile || ''}`.toLowerCase().includes(query.toLowerCase()));
  const statusLabel = (s: string) => s === 'active' ? 'Active' : s === 'abandoned' ? 'Abandoned' : s === 'recovered' ? 'Recovered' : 'Expired';
  const updateStatus = async (status: string) => { if (!selected || !editable) return; const result = await supabase.from('carts').update({ status }).eq('id', selected.id); if (result.error) alert(result.error.message); else { setSelected({ ...selected, status }); refresh(); } };
  if (selected) { const items = Array.isArray(selected.items) ? selected.items : []; return <Page title="Abandoned cart detail" sub="Customer, products, value and saved delivery information."><button className="outline" onClick={() => setSelected(null)}>← Back to abandoned carts</button><div className="detail-summary customer-summary"><article><Users /><div><small>Customer</small><b>{selected.customer_name || 'Guest customer'}</b><span>{selected.email || selected.mobile || 'No contact saved'}</span></div></article><article><IndianRupee /><div><small>Cart value</small><b>₹{Number(selected.subtotal || 0).toLocaleString('en-IN')}</b><span>{statusLabel(selected.status)}</span></div></article><article><Clock3 /><div><small>Activity</small><b>{selected.created_at ? new Date(selected.created_at).toLocaleString('en-IN') : '—'}</b><span>Last activity: {selected.updated_at ? new Date(selected.updated_at).toLocaleString('en-IN') : '—'}</span></div></article></div><Panel title="Products and quantities"><div className="table-wrap"><table><thead><tr><th>Product</th><th>Quantity</th><th>Current price</th><th>Value</th></tr></thead><tbody>{items.length ? items.map((i: any, n: number) => <tr key={i.product_id || n}><td>{i.name || i.product_name || i.sku || i.product_id || 'Product'}</td><td>{i.quantity || 0}</td><td>₹{Number(i.price || i.current_price || 0).toLocaleString('en-IN')}</td><td>₹{(Number(i.price || i.current_price || 0) * Number(i.quantity || 0)).toLocaleString('en-IN')}</td></tr>) : <tr><td colSpan={4} className="muted">No items recorded.</td></tr>}</tbody></table></div></Panel><Panel title="Saved delivery information"><p>{selected.pincode ? `PIN ${selected.pincode}` : 'No delivery PIN saved.'}</p></Panel><div className="status-actions"><b>Status</b>{['active','abandoned','recovered','expired'].map(s => <button key={s} className={selected.status === s ? 'primary' : 'outline'} disabled={!editable} onClick={() => updateStatus(s)}>{statusLabel(s)}</button>)}</div></Page>; }
  return <Page title="Abandoned carts" sub="Track cart activity and recovery status without sending reminders or discounts."><div className="toolbar"><label><Search /><input placeholder="Search cart, customer or contact…" value={query} onChange={e => setQuery(e.target.value)} /></label></div><div className="table-wrap"><table><thead><tr><th>Cart ID</th><th>Customer / contact</th><th>Items</th><th>Cart value</th><th>Created</th><th>Last activity</th><th>Time abandoned</th><th>View</th></tr></thead><tbody>{rows.map((c: any) => <tr key={c.id}><td><b>{String(c.id).slice(0, 8)}</b></td><td>{c.customer_name || 'Guest'}<br /><span className="muted">{c.email || c.mobile || '—'}</span></td><td>{Array.isArray(c.items) ? c.items.reduce((n: number, i: any) => n + Number(i.quantity || 0), 0) : '—'}</td><td>₹{Number(c.subtotal || 0).toLocaleString('en-IN')}</td><td>{c.created_at ? new Date(c.created_at).toLocaleString('en-IN') : '—'}</td><td>{c.updated_at ? new Date(c.updated_at).toLocaleString('en-IN') : '—'}</td><td>{c.status === 'abandoned' ? (c.updated_at ? new Date(c.updated_at).toLocaleString('en-IN') : '—') : '—'}</td><td><button className="view-order" onClick={() => setSelected(c)}><Eye /> View</button></td></tr>)}</tbody></table></div></Page>;
}
