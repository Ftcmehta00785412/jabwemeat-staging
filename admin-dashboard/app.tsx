...
import {
  ...
  History, Scale, Box, Eye, CalendarClock
} from 'lucide-react';
...
function Orders({ orders, refresh, editable, fullAccess, slots }: any) {
  ...
  const [detail, setDetail] = useState<any>({ items: [], payments: [], assignments: [], notes: [], timeline: [], executives: [] });
  ...
  const loadDetail = async (order: any) => {
    ...
    const failed = [items, payments, assignments, notes, timeline, executives].find(x => x.error);
    if (failed?.error) setActionError(failed.error.message);
    setDetail({ items: items.data || [], payments: payments.data || [], assignments: assignments.data || [], notes: notes.data || [], timeline: timeline.data || [], executives: executives.data || [] });
    setDetailBusy(false);
  };
  ...
  if (selected) return <OrderDetails order={selected} detail={detail} busy={detailBusy} editable={editable} fullAccess={fullAccess} actionError={actionError} onBack={() => { setSelected(null); setActionError(''); }} onStatus={changeStatus} onAssign={assign} onPrep={savePrep} note={note} setNote={setNote} onAddNote={addNote} />;

  return <Page title="Order management" sub="Control fulfilment from confirmation through final delivery.">
...
function OrderDetails({ order, detail, busy, editable, fullAccess, actionError, onBack, onStatus, onAssign, onPrep, note, setNote, onAddNote }: any) {
