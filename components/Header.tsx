...
type Props = { language: Language; setLanguage:(v:Language)=>void; view: AppView; setView:(v:AppView)=>void; onCategory:(v:string)=>void; pincode:string; setPincode:(v:string)=>void; search:string; setSearch:(v:string)=>void; cartCount:number; onCart:()=>void; onAccount:()=>void; accountEmail?:string|null };
...
export const Header: React.FC<Props> = ({language,setLanguage,view,setView,onCategory,pincode,setPincode,search,setSearch,cartCount,onCart,onAccount,accountEmail}) => {
 ...
 return <>
  ...
  <header className="site-header">
   <div className="header-main">
    ...
    <div className="language-toggle" aria-label="Language"><button className={language==='en'?'selected':''} onClick={()=>setLanguage('en')}>EN</button><button className={language==='hi'?'selected':''} onClick={()=>setLanguage('hi')}>हिं</button><button className={language==='bn'?'selected':''} onClick={()=>setLanguage('bn')}>বাং</button></div><nav className="header-actions"><button className="nav-action" onClick={onAccount} aria-label="Open account"><UserRound/><span>{accountEmail ? 'Account' : 'Account'}</span></button><button className="cart-button" onClick={onCart}><ShoppingCart/><span>Cart</span>{cartCount>0&&<b>{cartCount}</b>}</button></nav>
   </div>
