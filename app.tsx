...
const App: React.FC = () => {
 ...
 useEffect(()=>{let alive=true;supabase.auth.getSession().then(({data})=>{if(alive)setUser(data.session?.user||null);});const {data:{subscription}}=supabase.auth.onAuthStateChange((_event,session)=>{if(alive)setUser(session?.user||null);});return()=>{alive=false;subscription.unsubscribe();};},[]);
 ...
 useEffect(()=>{let alive=true;const load=async()=>{const [p,c,s,settings]=await Promise.all([supabase.from('products').select('*,categories(name)').eq('active',true).order('created_at'),supabase.from('categories').select('name').eq('active',true).order('sort_order'),supabase.from('delivery_slots').select('id,label').eq('active',true).order('sort_order'),supabase.from('store_settings').select('key,value')]);if(!alive)return;if(!p.error&&p.data?.length){const liveProducts=p.data.map(adaptProduct);setProducts(liveProducts);setCart(previous=>previous.flatMap(item=>{const live=liveProducts.find(product=>product.id===item.id||product.sku===item.sku);return live?[{...live,quantity:Math.max(1,Math.min(item.quantity,live.stock))}]:[];}));}if(!c.error&&c.data?.length)setCategories(['All',...c.data.map((x:any)=>x.name)]);if(!s.error&&s.data?.length)setSlots(s.data as Slot[]);if(!settings.error){const pin=(settings.data||[]).find((x:any)=>x.key==='serviceable_pins');if(Array.isArray(pin?.value)&&pin.value.length){setServiceablePins(pin.value.map(String));setPincode(prev=>pin.value.includes(prev)?prev:String(pin.value[0]));}}setLoading(false);};load();
